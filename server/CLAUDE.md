...
Данных нет — есть статический снапшот. `server/data/products.json` (загружается через `createRequire` в `server/data/products.js`) и картинки в `public/product-images/` генерируются скриптом `server/scripts/seed.mjs` из датасета DummyJSON; скрипт скачивает изображения и переписывает URL на локальные пути, чтобы приложение не зависело от `dummyjson.com` в рантайме. Файл `server/data/products.json` исключён из проверок Biome.
